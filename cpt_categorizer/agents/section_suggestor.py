"""Section Suggestor agent: suggest new section (key + description) when tagging yields "others"; check store before LLM."""
...
SECTION_SUGGEST_PROMPT_TEMPLATE = """You are a medical classification expert. The following CPT procedure description did not match any existing section.

Existing section keys (do not reuse): {existing_keys}

CPT description:
{text_description}

Propose exactly one new section key (snake_case, e.g. wound_care) and a short description for it. The key must not be one of the existing keys above.
"""


def _context_for_description(text_description: str) -> str:
    """Stable context for deduplication: same description -> same context."""
...
class SectionSuggestorAgent:
    """Suggests a new section (key + description) when section tagging yields 'others'; checks store before LLM."""
    ...
    def __init__(
        ...
        self._existing_keys = ", ".join(sorted(section_schema.keys()))

    def suggest_section(self, text_description: str, source: str = "") -> Optional[dict[str, Any]]:
        """
        Suggest a new section for a CPT description that got "others". Checks store before LLM;
        returns existing suggestion if same context or same key already in store.
        """
        if not text_description or not text_description.strip():
            return None

        context = _context_for_description(text_description)
        start = time.time()

        # 1. Check store by context: already suggested for this description?
        ...
        if matching_by_context:
            ...
            _log_usage(
                ...
                success=True,
                error_message="",
                description="section_suggest_store_hit",
            )
            return matching_by_context[0]

        # 2. Call LLM to get suggested_key and suggested_description
        prompt = SECTION_SUGGEST_PROMPT_TEMPLATE.format(
            existing_keys=self._existing_keys,
            text_description=text_description,
        )
        function_spec = {
            "name": "suggest_section",
            "description": "Return one new section key (snake_case) and short description.",
            "parameters": {
                "type": "object",
                "properties": {
                    "suggested_key": {
                        "type": "string",
                        "description": "New section key in snake_case, not in existing keys.",
                    },
                    "suggested_description": {
                        "type": "string",
                        "description": "Short description of the new section.",
                    },
                },
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                function_call={"name": "suggest_section"},
            )
            ...
            if not suggested_key:
                suggested_key = to_snake_case("new_section")
            success = True
        ...
        if existing_reusable:
            ...
            _log_usage(
                ...
                success=True,
                error_message="",
                description="section_suggest_duplicate_key",
            )
